...
<div x-data="{ show: false }" x-show="show" x-on:open-product-create-modal.window="show = true"
    x-on:close-product-create-modal.window="show = false,$dispatch('product-create-modal-closed')" class="relative z-10"
    ...
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div
                ...
                {{-- modal content --}}
                <div class="bg-white" @click.outside="$dispatch('close-product-create-modal')" wire:ignore.self>
                    {{-- header --}}
                    <div class="flex items-center justify-between p-4 border-b  dark:border-gray-600">
                        ...
                        <button type="button" @click="$dispatch('close-product-create-modal')"
                    ...
                    <form class="p-4 space-y-2" action="#">
                        <div class="grid gap-4 mb-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select wire:model.live='createForm.brand' id="brand"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                    ...
                                    @foreach ($brands ?? [] as $brand)
                                        <option wire:key='brand-{{ $brand->id }}' value="{{ $brand->id }}">
                                            {{ $brand->name }}</option>
                                    @endforeach
                                </select>
                                @error('createForm.brand')
                                    <p id="standard_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">
                                        {{ $message }}</p>
                                @enderror
                            </div>
                            <div class="sm:col-span-2">
                                ...
                                <input wire:model='createForm.price' type="number" value="399" name="price"
                                    id="price"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                @error('createForm.price')
                                    <p id="standard_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">
                                        {{ $message }}</p>
                                @enderror
                            </div>
                            <div class="sm:col-span-2">
                                ...
                                <textarea wire:model.blur='createForm.description' id="description" rows="5"
                        ...
                        <div class="flex items-center justify-between gap-1">
                            ...
                            <button wire:click.prevent='resetCreateFormFields'
                                class="text-white bg-red-500 px-3 py-1.5 rounded-lg"
                                data-popover-target="popover-hover" data-popover-trigger="hover">
