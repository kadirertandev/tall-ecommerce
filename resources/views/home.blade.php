<x-layout>
    <div class="flex gap-4 my-4 main-container">
        <div class="w-9/12">
            <h1 class="mb-2 text-3xl font-thin font-roboto">{{ __('frontend.weekly_deals') }}</h1>
            <div id="controls-carousel" class="relative w-full" data-carousel="static">
                        ...
                        @if ($product->product && $product->product->category)
                            <div class="hidden duration-200 ease-linear" data-carousel-item>
                                <a href="{{ route('products.show', ['category_slug' => $product->product->category->slug, 'product_slug' => $product->product->slug]) }}"
                                    class="flex items-center bg-gray-300 h-96 justify-evenly">
                                    ...
                                    <div>
                                        ...
                                        <h2
                                            class="text-2xl font-semibold line-through font-roboto text-zinc-600 decoration-2">
                ...
                <!-- Slider controls -->
                <button type="button"
                    class="absolute top-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer start-0 group focus:outline-hidden"
                    data-carousel-prev>
                    <span
                        class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-2 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-hidden">
                ...
                <button type="button"
                    class="absolute top-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer end-0 group focus:outline-hidden"
                    data-carousel-next>
                    <span
                        class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-2 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-hidden">
                        <svg class="w-4 h-4 text-white dark:text-gray-800 rtl:rotate-180" aria-hidden="true"
        ...
        <div class="w-3/12">
            <h1 class="mb-2 text-3xl font-thin font-roboto">{{ __('frontend.daily_deals') }}</h1>
            <div id="controls-carousel" class="relative w-full" data-carousel="static">
                <!-- Carousel wrapper -->
                <div class="relative h-56 overflow-hidden rounded-lg md:h-96 ">
                    @foreach ($daily_deal_products as $product)
                        @if ($product->product && $product->product->category)
                            <div class="hidden duration-200 ease-linear" data-carousel-item>
                                <a href="{{ route('products.show', ['category_slug' => $product->product->category->slug, 'product_slug' => $product->product->slug]) }}"
                                    class="flex flex-col items-center justify-center bg-gray-300 h-96">
                                    <img class="h-48 max-w-80" src="{{ asset('storage/' . $product->product->image) }}"
                                        alt="">
    ...
    <div class="pb-4 main-container">
        <h1 class="mt-2 mb-4 text-4xl font-thin font-roboto">Popular Brands</h1>
        <div class="grid grid-cols-2 md:grid-cols-3 gap-2 *:text-white">
            @foreach ($popularBrands as $brand)
                <a href="{{ route('brand-slug', ['slug' => $brand->slug]) }}"
                    class="flex items-center justify-center gap-2 p-2 bg-neutral-300">
