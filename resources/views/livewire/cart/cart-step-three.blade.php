<x-cart-content-layout title="Confirm Your Order" :step="3">
    <x-slot:content>
        <div class="space-y-6">
    ...
    <x-slot:rightSideContent>
        ...
        <div for="hosting-small-{{ $this->selectedAddress?->id }}"
            class='inline-flex items-center justify-between w-full p-5  bg-white border  rounded-lg cursor-pointer  peer-checked:border-blue-600 peer-checked:text-blue-600 hover:text-gray-600 hover:bg-gray-100'>
            <div class="flex-1 block">
                <h1 class="w-full text-lg font-semibold">{{ $this->selectedAddress?->title }}
                </h1>
                <h2 class="w-full">{{ $this->selectedAddress?->neighborhood }}</h2>
                <p class="w-full">{{ $this->selectedAddress?->address_line }}</p>
                <h2 class="w-full">{{ $this->selectedAddress?->district }} /
                    {{ $this->selectedAddress?->city }}</h2>
            </div>
